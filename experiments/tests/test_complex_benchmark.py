...
核心场景�?1. 多走廊交叉（结构切换�?2. 动态障碍（时序预测�?3. 多目标切换（目标变化时结构复用）
4. 大规模迷宫（50x50，结构层次）
5. 非对称地形（Finsler 优势�?6. 信息缺失（部分观测下的结构推断）

核心指标�?- 结构发现�?= 发现的独特结构数 / 场景中的真实结构�?- 结构复用�?= 复用次数 / 总步�?- 首次发现步数 = 发现第一个稳定结构需要的步数
...
class ComplexMazeEnv:
    ...
    场景类型�?    - multi_corridor: 多走廊交�?    - dynamic_obstacles: 动态障�?    - multi_goal: 多目标切�?    - large_scale: 大规模迷�?    - asymmetric: 非对称地�?    - partial_obs: 信息缺失
    """
    ...
    def __init__(self, width: int = 30, height: int = 30, maze_type: str = 'multi_corridor'):
        ...
        # 生成障碍�?        self.obstacles = self._generate_obstacles()
        self.dynamic_obstacles = set()  # 动态障�?        self.goals = [self.goal]  # 多目�?        self.current_goal_idx = 0
    ...
    def _generate_obstacles(self) -> Set[Tuple[int, int]]:
        ...
        if self.maze_type == 'multi_corridor':
            # 多走廊交�?            # 主走�?            for x in range(self.width):
                obstacles.add((x, 0))
            ...
            # 动态障碍场�?            for x in range(self.width):
                obstacles.add((x, 0))
                obstacles.add((x, self.height - 1))
            # 静态障�?            for y in range(5, self.height - 5):
                obstacles.add((self.width // 2, y))

        elif self.maze_type == 'multi_goal':
            # 多目标场�?            for x in range(self.width):
                obstacles.add((x, 0))
                obstacles.add((x, self.height - 1))
            self.goals = [
            ...
            for y in range(self.height):
                obstacles.add((0, y))
                obstacles.add((self.width - 1, y))
            # 随机内部障碍�?0%密度�?            for x in range(2, self.width - 2):
            ...
            # 非对称地�?            for x in range(self.width):
                obstacles.add((x, 0))
                obstacles.add((x, self.height - 1))
            # 左侧密集障碍
    ...
    def step(self, action: str) -> Tuple[Dict, float, bool]:
        ...
        # 边界检�?        if not (0 <= new_pos[0] < self.width and 0 <= new_pos[1] < self.height):
            new_pos = self.position

        # 障碍检�?        if new_pos in self.obstacles or new_pos in self.dynamic_obstacles:
            new_pos = self.position

        self.position = new_pos

        # 动态障碍更�?        if self.maze_type == 'dynamic_obstacles':
            self._update_dynamic_obstacles()

        # 多目标切�?        if self.maze_type == 'multi_goal':
    ...
    def _update_dynamic_obstacles(self):
        ...
        if self.position[0] % 3 == 0:
            # 清除旧动态障�?            self.dynamic_obstacles.clear()
            # 添加新动态障�?            for y in range(2, self.height - 2):
...
class SSFRNavigator:
    ...
    def __init__(self, env: ComplexMazeEnv,
        ...
        # 初始化SSFR（带复用机制�?        self.ssfr = SSFREnhanced(
            width=env.width,
            height=env.height,
    ...
    def solve(self, state: Dict) -> str:
        ...
        if best_structures:
            ...
            # 空间切换时清除路径缓�?            if self.current_space_name != recommended_space:
                self._path_valid = False
                self.space_switches += 1

            self.current_space_name = recommended_space

        # 使用选中的空间求�?        space = self._get_space(self.current_space_name)

        try:
            # 只在空间切换时更新（避免每步重复更新�?            if not self._path_valid:
                space.update_from_observation(pos, observation)

            # 检查缓存路径是否有�?            if self._is_path_valid(self._cached_path, obstacles, goal):
                # 缓存命中！直接沿路径�?                self.cache_hits += 1
            ...
            # 缓存未命中，重新规划（A*调用�?            self.astar_calls += 1
            if not self._path_valid:
                ...
                if path and len(path) > 1:
                    ...
                    # A*失败，回退到贪�?                    next_pos = greedy_step(
                        space, pos, goal, obstacles,
                        self.env.width, self.env.height
                    )
                    if next_pos:
...
def run_benchmark(maze_type: str = 'multi_corridor',
    ...
    # 创建导航�?    nav = SSFRNavigator(env, use_theory_boundary=True, reuse_threshold=reuse_threshold)
...
def compare_reuse_scenarios():
    ...
    for scenario in scenarios:
        ...
        # 有复�?        results_with, stats_with = run_benchmark(
