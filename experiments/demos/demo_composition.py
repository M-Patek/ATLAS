...
展示 ProductSpace, HierarchicalSpace, MixedSpace 的实际应�?"""
...
# 垂直墙（有缺口）
for y in range(5, 26):
    if abs(y - 15) > 2:  # �?y=15 附近留缺�?        obstacles.add((25, y))
...
for name, explore_w, nav_w in weight_configs:
    ...
    # 初始�?    product.update_from_observation(start, {
...
# 创建层次空间
scale_factor = 5  # 全局空间比局部粗5�?global_width = width // scale_factor
...
# 初始�?hierarchical.update_from_observation(start, {
    'obstacles': list(obstacles),
    'goal_position': goal
})
...
# 使用求解�?solver_h = GeodesicSolver(hierarchical)
...
# 基于uncertainty动态切�?mixed = MixedSpace(width, height, [
...
# 起点（高uncertainty区域�?position = start
uncertainty = 0.8  # 初始高不确定�?
path_taken = [position]
active_spaces = []

for step in range(50):  # 最�?0�?    if position == goal:
        break
    ...
    # 规划下一�?    mixed.update_from_observation(position, context)
